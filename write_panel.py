import pathlib
src = r'''import { useState, useEffect, useCallback, useMemo, useRef } from 'react'
import {
...
function PluginRow({
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(handleSave, 350)
    return () => window.clearTimeout(t)
  }, [isExpanded, localName, localUrl, localRequestStyle, localAuth, localAuthConfig, handleSave, plugin])
              ...
              <div className="inline-flex rounded-md border border-border overflow-hidden w-fit">
                {REQUEST_STYLES.map(rs => (
                  <button
                    key={rs.value}
                    type="button"
                    onClick={() => setLocalRequestStyle(rs.value)}
...
'''
path = pathlib.Path('frontend/src/components/overlay/DataPluginsPanel.jsx')
