# lrs-cleanup.ps1 - Kill any processes holding LRS ports (6369, 5299) or named lrs_capture.
...
if ($ShowDetails) {
    ...
    if ($liveRemaining -eq 0 -and $ghostSockets -eq 0) {
        Write-Host '  All clear. Ports 6369 and 5299 are free.'
    } elseif ($liveRemaining -gt 0) {
        Write-Host "  WARNING: $liveRemaining live process(es) still holding ports - manual intervention needed."
        Write-Host '  Run: Get-NetTCPConnection -LocalPort 6369,5299 | Select LocalPort,State,OwningProcess'
    } else {
        ...
        for ($i = 1; $i -le 30; $i++) {
            Start-Sleep -Seconds 1
            $stillThere = Get-NetTCPConnection -LocalPort 6369 -State Listen -ErrorAction SilentlyContinue
            if (-not $stillThere) {
