...
export default function EventsTabContent({
  ...
  const { params } = useContext(HighlightContext)
  const scrollContainerRef = useRef(null)
  const [showSlideIn, setShowSlideIn] = useState(false)
  const wasAnalyzingRef = useRef(false)
  const slideTimerRef = useRef(null)

  // When analysis completes, trigger slide-in animation and auto-scroll to top
  useEffect(() => {
    if (isAnalyzing) {
    ...
    if (wasAnalyzingRef.current && !isAnalyzing && events.length > 0) {
      setShowSlideIn(true)
    ...
    <div className="flex flex-col h-full">
      ...
      {isAnalyzing ? (
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          {/* Particle overlay fills the entire available sidebar space */}
          <div className="relative flex-1 min-h-0 overflow-hidden">
            <EventParticleOverlay feedEvents={feedEvents || []} isAnalyzing={isAnalyzing} />
          ...
          <div className="px-4 py-3 border-t border-border-subtle shrink-0">
            {!isScanning && progress != null && (
              <div className="w-full">
                ...
                {progress.message && (
                  <span className="text-xxs text-text-disabled mt-1.5 block truncate">
                    {progress.message}
                  </span>
                )}
              </div>
            ...
            return sorted.map((ev, idx) => {
              ...
              return (
                <div key={`e-${eventId}`}
                     ...
                     style={showSlideIn ? {
                       animation: `slideUp 0.5s ease-out ${idx * 30}ms forwards`,
                       opacity: 0,
                  ...
                  >
                    ...
                    <div className="flex items-center px-2 py-1.5">
                      {(() => {
                        ...
                        const effectiveEnd = ev.end_time_seconds + paddingAfter - raceStart
                        return (
                          <span className="text-xxs text-text-disabled font-mono whitespace-nowrap">
